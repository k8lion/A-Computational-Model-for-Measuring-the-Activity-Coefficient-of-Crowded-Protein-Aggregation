...
% curr = size(y,1);
%load('randwalk_output_test12.mat');
newttB = [];
newc = [];
newtime = [];
trials = 12;
%existingTrials = length(c);

parfor i = 1:trials
    ts = tic;
    [times,locs,types,rs,orients,vf1,tt,tD,eD,bP,nB,newttB(i,:),newc(i)] = RandWalkLM2(18,18,5,89,0,100000,100000000000,0,20);
    newtime(i) = toc(ts);
end
